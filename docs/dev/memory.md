...
- Tests: `test/integration/plots.jl` (13 smoke tests, `isa Figure`) + `CairoMakie` in `test/Project.toml`, wired into `runtests.jl`.
...
- Clarified in `test/integration/plots.jl` that the energy-drift smoke test feeds a pointwise error series as a stand-in (`compute_drift` is unavailable in the pinned GeometricSolutions 0.6.4).
