...
function podeproblem(q₀ = q₀, p₀ = p₀; timespan = DEFAULT_TIMESPAN,
    ...
    PODEProblem(pendulum_pode_v, pendulum_pode_f, timespan,
        timestep, q₀, p₀; parameters = parameters)
end

function hodeproblem(q₀ = q₀, p₀ = p₀; timespan = DEFAULT_TIMESPAN,
        timestep = DEFAULT_TIMESTEP, parameters = default_parameters())
    @assert length(q₀) == length(p₀) == 1
    HODEProblem(pendulum_pode_v, pendulum_pode_f, hamiltonian,
...
function hodeensemble(
    ...
    HODEEnsemble(pendulum_pode_v, pendulum_pode_f, hamiltonian, timespan,
        timestep, samples.q, samples.p; parameters = parameters)
end

# Ensemble over a vector of parameter sets sharing a single initial condition (q₀, p₀).
function hodeensemble(q₀, p₀, parameters::AbstractVector{<:NamedTuple};
        timespan = DEFAULT_TIMESPAN, timestep = DEFAULT_TIMESTEP)
    @assert length(q₀) == length(p₀) == 1
    HODEEnsemble(pendulum_pode_v, pendulum_pode_f, hamiltonian,
        timespan, timestep, q₀, p₀; parameters = parameters)
end
...
function iodeproblem(q₀ = x₀, p₀ = p₀_iode; timespan = DEFAULT_TIMESPAN,
        timestep = DEFAULT_TIMESTEP, parameters = default_parameters())
...
function idaeproblem(q₀ = x₀, p₀ = p₀_iode, λ₀ = zero(q₀); timespan = DEFAULT_TIMESPAN,
        timestep = DEFAULT_TIMESTEP, parameters = default_parameters())
