...
const initial_conditions = [(
                                q = [sin(π * i₁ / 5), cos(π * i₂ / 5),
                                    sin(π * j₁ / 5 + π / 2), sin(π * j₂ / 5),
                                    cos(π * k₁ / 5), sin(π * k₂ / 5 + π / 2)],
                                p = [sin(π * iᵖ₁ / 5), cos(π * iᵖ₂ / 5),
                                    sin(π * jᵖ₁ / 5 + π / 2), sin(π * jᵖ₂ / 5),
                                    cos(π * kᵖ₁ / 5), sin(π * kᵖ₂ / 5 + π / 2)]
                            )
                            for i₁ in 1:2,
i₂ in 1:2, j₁ in 1:2, j₂ in 1:2, k₁ in 1:2, k₂ in 1:2, iᵖ₁ in 1:2,
iᵖ₂ in 1:2, jᵖ₁ in 1:2, jᵖ₂ in 1:2, kᵖ₁ in 1:2, kᵖ₂ in 1:2] |> _reshape
@doc raw"""
...
T(p::AbstractVector, params::NamedTuple) = (p[1] ^ 2 + p[2] ^ 2) / (2 * params.m₁) +
                                           (p[3] ^ 2 + p[4] ^ 2) / (2 * params.m₂) +
                                           (p[5] ^ 2 + p[6] ^ 2) / (2 * params.m₃)
V(q::AbstractVector, params::NamedTuple) = -params.G * params.m₁ * params.m₂ /
                                           √((q[1] - q[3]) ^ 2 + (q[2] - q[4]) ^ 2) -
                                           params.G * params.m₂ * params.m₃ /
                                           √((q[3] - q[5]) ^ 2 + (q[4] - q[6]) ^ 2) -
                                           params.G * params.m₁ * params.m₃ /
                                           √((q[1] - q[5]) ^ 2 + (q[2] - q[6]) ^ 2)
...
function hodeproblem(
        q₀ = initial_condition.q, p₀ = initial_condition.p; timespan = DEFAULT_TIMESPAN,
...
function lodeensemble(
        q₀ = initial_condition.q, p₀ = initial_condition.p; timespan = DEFAULT_TIMESPAN,
        timestep = DEFAULT_TIMESTEP, parameters = default_parameters())
    eqs = functions(lagrangian_system(_parameters(parameters)))
    LODEEnsemble(eqs.ϑ, eqs.f, eqs.g, eqs.ω, eqs.L, timespan,
        timestep, q₀, p₀; v̄ = v̄, parameters = parameters)
end
