...
Dynamics of interacting point vortices in the plane. This module implements a *deformed*
point-vortex model: the non-canonical symplectic structure carries an additional factor
``S = 1 + x^2 + y^2`` (a curved background), so it is **not** the plain Kirchhoff–Helmholtz system.
The motion is generated by a logarithmic interaction Hamiltonian (`hamiltonian`) together with a
non-canonical one-form whose components are `ϑ₁`–`ϑ₄` and which encodes the vortex circulations.
Explicit (`odeproblem`), implicit/variational (`iodeproblem`, `lodeproblem_formal_lagrangian`) and
differential-algebraic (`idaeproblem`) formulations are provided, and `angular_momentum` is a
conserved quantity. See also `PointVorticesLinear` for the linearised one-form variant.
"""
...
# The vortex circulations γ₁, γ₂ are fixed structural constants of this model instance,
# so there are no tunable parameters and default_parameters returns an empty NamedTuple.
default_parameters(::Type{T} = Float64) where {T} = NamedTuple()

const reference_solution = [
    0.18722529318641928, 0.38967432450068706, 0.38125332930294187, 0.4258020604293123]
...
ϑ₃(q) = - γ₂ * q[4] * S(q[3], q[4]) / 2
ϑ₄(q) = + γ₂ * q[3] * S(q[3], q[4]) / 2
...
function dHd1(t, q)
    + γ₁ * γ₂ * dSdx(q[1], q[2]) * S(q[3], q[4]) * log((q[1] - q[3])^2 + (q[2] - q[4])^2) /
    (2π) +
    γ₁ * γ₂ * S(q[1], q[2]) * S(q[3], q[4]) * (q[1] - q[3]) /
    ((q[1] - q[3])^2 + (q[2] - q[4])^2) / π
end

function dHd2(t, q)
    + γ₁ * γ₂ * dSdy(q[1], q[2]) * S(q[3], q[4]) * log((q[1] - q[3])^2 + (q[2] - q[4])^2) /
    (2π) +
    γ₁ * γ₂ * S(q[1], q[2]) * S(q[3], q[4]) * (q[2] - q[4]) /
    ((q[1] - q[3])^2 + (q[2] - q[4])^2) / π
end

function dHd3(t, q)
    + γ₁ * γ₂ * dSdx(q[3], q[4]) * S(q[1], q[2]) * log((q[1] - q[3])^2 + (q[2] - q[4])^2) /
    (2π) -
    γ₁ * γ₂ * S(q[1], q[2]) * S(q[3], q[4]) * (q[1] - q[3]) /
    ((q[1] - q[3])^2 + (q[2] - q[4])^2) / π
end
...
function dHd4(t, q)
    + γ₁ * γ₂ * dSdy(q[3], q[4]) * S(q[1], q[2]) * log((q[1] - q[3])^2 + (q[2] - q[4])^2) /
    (2π) -
    γ₁ * γ₂ * S(q[1], q[2]) * S(q[3], q[4]) * (q[2] - q[4]) /
    ((q[1] - q[3])^2 + (q[2] - q[4])^2) / π
end
...
function idaeproblem(q₀ = q₀, p₀ = ϑ(q₀), λ₀ = zero(q₀); timespan = DEFAULT_TIMESPAN,
        timestep = DEFAULT_TIMESTEP, parameters = default_parameters())
